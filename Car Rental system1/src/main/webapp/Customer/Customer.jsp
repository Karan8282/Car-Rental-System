  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        <li class="nav-item">
          <a class="nav-link active" aria-current="page" href="Customer.jsp?action=home" id="Dashboard" onclick=change(event)>Home</a>
        </li>
        <li class="nav-item">
          <a class="nav-link"  href="Customer.jsp?action=MyBooking" >My Bookings</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="Customer.jsp?action=ReturnCar">Return Car</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="Customer.jsp?action=Contact">Contact</a>
        </li>
      </ul>
      <div class="dropdown">
...
<%=session.getValue("firstName") %>   
  ...
  <path d="M9.405 1.05c-.413-1.4-2.397-1.4-2.81 0l-.1.34a1.464 1.464 0 0 1-2.105.872l-.31-.17c-1.283-.698-2.686.705-1.987 1.987l.169.311c.446.82.023 1.841-.872 2.105l-.34.1c-1.4.413-1.4 2.397 0 2.81l.34.1a1.464 1.464 0 0 1 .872 2.105l-.17.31c-.698 1.283.705 2.686 1.987 1.987l.311-.169a1.464 1.464 0 0 1 2.105.872l.1.34c.413 1.4 2.397 1.4 2.81 0l.1-.34a1.464 1.464 0 0 1 2.105-.872l.31.17c1.283.698 2.686-.705 1.987-1.987l-.169-.311a1.464 1.464 0 0 1 .872-2.105l.34-.1c1.4-.413 1.4-2.397 0-2.81l-.34-.1a1.464 1.464 0 0 1-.872-2.105l.17-.31c.698-1.283-.705-2.686-1.987-1.987l-.311.169a1.464 1.464 0 0 1-2.105-.872l-.1-.34zM8 10.93a2.929 2.929 0 1 1 0-5.86 2.929 2.929 0 0 1 0 5.858z"/>
 		     ...
 		     <a href="Customer.jsp?action=EditProfile" style="text-decoration: none;color:#0e0f0f">Edit Profile</a><hr>
 	</li>
  ...
  <path fill-rule="evenodd" d="M15.854 8.354a.5.5 0 0 0 0-.708l-3-3a.5.5 0 0 0-.708.708L14.293 7.5H5.5a.5.5 0 0 0 0 1h8.793l-2.147 2.146a.5.5 0 0 0 .708.708l3-3z"/>
     	 ...
     	 <a href="Customer.jsp?action=Logout" style="text-decoration: none;color:#0e0f0f">Log Out</a>
     </li>
    </ul>
  </div>
    </div>
...
<div class="container">
    ...
    <% 
    	...
    	if(request.getParameter("action").equals("ConfirmBooking"))
		{
			%>
			<%@ include file="BookingConfirmation.jsp" %> 
			<%
		}if(request.getParameter("action").equals("MyBooking"))
		{
			%>
			<%@ include file="MyBooking.jsp" %> 
			<%
		}
		if(request.getParameter("action").equals("ReturnCar"))
		{
			%>
			<%@ include file="ReturnCar.jsp" %> 
			<%
		}
		if(request.getParameter("action").equals("CarReturned"))
		{
			%>
			<%@ include file="carReturned.jsp" %> 
			<%
		}
